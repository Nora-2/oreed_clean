...
import 'package:oreed_clean/features/technicalforms/data/models/technican_detailes_mode.dart';
import 'package:oreed_clean/features/technicalforms/data/models/technican_response_model.dart';
...
class TechnicianRemoteDataSource {
  ...
  Future<List<StateEntity>> getStates() async {
    ...
    final res = await http.get(
      Uri.parse('$baseUrl/country_states/1'),
  ...
  Future<List<CityEntity>> getCities(int stateId) async {
    final res = await http.get(
      Uri.parse('$baseUrl/state_cities/$stateId'),
      headers: _buildHeaders(),
    );
